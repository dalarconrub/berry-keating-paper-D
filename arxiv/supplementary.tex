...
\subsection{Extended Data Figure 3: Ab initio $c$ versus $R_3$ threshold}

See Extended Data Figure~3.
The scan over $R_3^{\mathrm{GUE}}$ threshold shows $c_{\mathrm{rich}}$
increasing from $-0.37$ (threshold $0.05$) to $+1.23$ (threshold $0.30$)
to $+2.54$ (threshold $0.50$). The value $c = 1.23$ at threshold $0.30$
reproduces $99\%$ of $c_{\mathrm{emp}}$.
...
%% ================================================================
\section{Supplementary Note 1: Perturbative approaches to $c$ (29 attempts)}

We attempted 29 distinct perturbative approaches to derive $c$ from first
principles. All failed due to three structural obstructions:

\begin{table}[h]
\centering
\caption{Summary of 29 perturbative approaches. All blocked by one of
three obstructions.}
...
The exact kernel $K^{\mathrm{BK}} = \sqrt{\mathrm{sinc}^2 + \varepsilon\,\delta Y_2}$
bypasses Obstruction~1 because $\sqrt{a^2 + b}$ is smooth at $a = 0$.
However, computing $c$ from this kernel remains an open problem (Section~4 of main text).
The ab initio route via Conrey--Snaith $R_3$ circumvents all three obstructions
by working directly with the triple correlation [Paper~B].
